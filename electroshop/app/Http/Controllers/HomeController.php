...
class HomeController extends Controller
{
    ...
    public function status_order($customer_id){
        $order_details = DB::table('tbl_order as o')
        ->select(
            'od.product_name',
            'od.product_price',
            'od.product_sales_quantity',
            DB::raw('od.product_price * od.product_sales_quantity as total_price_per_product'),
            'o.order_id',
            'o.order_total',
            'o.order_status',
            's.*'
        )
        ->join('tbl_order_details as od', 'o.order_id', '=', 'od.order_id')
        ->join('tbl_shipping as s', 'o.shipping_id', '=', 's.shipping_id')
        ->where('o.customer_id', '=', $customer_id)
        ->get();
        // dd($order_details);
        return view('status_order')->with('order_details',$order_details);
    }
